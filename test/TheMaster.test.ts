...
const setupTest = async () => {
  ...
  await poolToken
    .connect(alice)
    .approve(theMaster.address, ethers.constants.MaxUint256);
  await poolToken
    .connect(bob)
    .approve(theMaster.address, ethers.constants.MaxUint256);
  await poolToken
...
describe("TheMaster", function () {
  ...
  it("overall test", async function () {
    ...
    await expect(theMaster.connect(dan).deposit(0, 1, 0)).to.be.revertedWith(
      "TheMaster: Deposit to your address"
    );
    await expect(theMaster.connect(dan).deposit(1, 1, 0)).to.be.revertedWith(
      "TheMaster: Deposit to your address"
    );
    await expect(theMaster.connect(dan).deposit(2, 1, 0)).to.be.revertedWith(
      "TheMaster: Not called by delegate"
    );
    await expect(theMaster.connect(dan).deposit(3, 1, 0)).to.be.revertedWith(
      "TheMaster: Use support func"
    );

    await expect(theMaster.connect(dan).support(0, 1, 0)).to.be.revertedWith(
      "TheMaster: Use deposit func"
    );
    await expect(theMaster.connect(dan).support(1, 1, 0)).to.be.revertedWith(
      "TheMaster: Use deposit func"
    );
    await expect(theMaster.connect(dan).support(2, 1, 0)).to.be.revertedWith(
      "TheMaster: Use deposit func"
    );
    ...
    await expect(() =>
      theMaster.connect(dan).desupport(3, 1)
    ).to.changeTokenBalances(maidCoin, [dan, bob], [toDan, toNFT]);
    ...
    await expect(() => nurse.connect(bob).claim(1)).to.changeTokenBalance(
      maidCoin,
      bob,
      r1
    );
    await expect(() => nurse.connect(carol).claim(2)).to.changeTokenBalance(
      maidCoin,
      carol,
      r2
    );
    await expect(() => nurse.connect(alice).claim(3)).to.changeTokenBalance(
      maidCoin,
      alice,
      r3
    );
    await expect(() => nurse.connect(carol).claim(5)).to.changeTokenBalance(
      maidCoin,
      carol,
      r5
    );
    await expect(() => nurse.connect(alice).claim(6)).to.changeTokenBalance(
      maidCoin,
      alice,
      r6
    );
    await expect(() => nurse.connect(bob).claim(7)).to.changeTokenBalance(
      maidCoin,
      bob,
      r7
    );
    await expect(() => nurse.connect(bob).claim(9)).to.changeTokenBalance(
      maidCoin,
      bob,
      r9
    );
