    ...
    context("new Maids", async () => {
        ...
        it("support, powerOf", async () => {
            ...
            await expect(maids.mint(power))
                .to.emit(maids, "Transfer")
                .withArgs(constants.AddressZero, admin.address, id)
            await expect(maids.support(id, token))
                .to.emit(maids, "Support")
                .withArgs(id, token)
            expect(await maids.powerOf(id)).to.eq(power.add(token.mul(await maids.lpTokenToMaidPower()).div(expandTo18Decimals(1))))
        })
        ...
        it("desupport, powerOf", async () => {
            ...
            await expect(maids.support(id, token))
                ...
                .withArgs(id, token)
            expect(await maids.powerOf(id)).to.eq(power.add(token.mul(await maids.lpTokenToMaidPower()).div(expandTo18Decimals(1))))
            await expect(maids.desupport(id, token))
                .to.emit(maids, "Desupport")
                .withArgs(id, token)
            expect(await maids.powerOf(id)).to.eq(power)
        })
