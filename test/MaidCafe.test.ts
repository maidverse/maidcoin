...
describe("MaidCafe", () => {
  ...
  beforeEach(async () => {
    ...
    maidCafe = (await deployContract(admin, MaidCafeArtifact, [
      maidCoin.address,
    ])) as MaidCafe;
  });
  ...
  context("new MaidCafe", async () => {
    ...
    it("enter", async () => {
      ...
      await maidCoin
        .connect(other)
        .approve(maidCafe.address, value0.mul(1000000));
      await expect(maidCafe.connect(other).enter(value1))
        .to.emit(maidCafe, "Enter")
        .withArgs(other.address, value1);

      expect(await maidCoin.balanceOf(other.address)).to.be.equal(
        value0.sub(value1)
      );
      ...
      expect(await maidCoin.balanceOf(maidCafe.address)).to.be.equal(
        value1.mul(2)
      );

      await expect(() =>
        maidCafe.connect(other).enter(value1)
      ).to.changeTokenBalance(maidCafe, other, value1.div(2));
      expect(await maidCafe.balanceOf(other.address)).to.be.equal(
        value1.mul(3).div(2)
      );
    });
    ...
    it("burn", async () => {
      ...
      await expect(() =>
        maidCafe.connect(other).leave(value0.sub(value1))
      ).to.changeTokenBalance(maidCoin, other, value0.sub(value1).mul(2));
    });
    ...
    it("permit", async () => {
      ...
      await expect(
        maidCafe.enterWithPermit(value, deadline, v, hexlify(r), hexlify(s))
      )
        .to.emit(maidCafe, "Enter")
        .withArgs(admin.address, value);
      expect(await maidCoin.allowance(admin.address, maidCafe.address)).to.eq(
        0
      );
